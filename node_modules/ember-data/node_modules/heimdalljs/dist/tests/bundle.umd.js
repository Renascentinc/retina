...
describe('HeimdallSession', function () {
  ...
  describe('reset()', function () {
    it('is callable', function () {
      ...
      expect$3(_typeof(session.reset)).to.equal('function');
      expect$3(function () {
        session.reset();
      }).to.not.throw();
    });
    ...
    it('is resettable', function () {
      var session = new HeimdallSession();
      var heimdall = new Heimdall(session);
      var tree = new HeimdallTree(heimdall);

      var _heimdall$registerMon = heimdall.registerMonitor('foo', 'a', 'b', 'c');

      var a = _heimdall$registerMon.a;
      var b = _heimdall$registerMon.b;
      var c = _heimdall$registerMon.c;

      var _heimdall$registerMon2 = heimdall.registerMonitor('bar', 'd', 'e', 'f');

      var d = _heimdall$registerMon2.d;
      var e = _heimdall$registerMon2.e;
      var f = _heimdall$registerMon2.f;


      var token = heimdall.start('a node!');
      ...
      var node = tree.root.nodes[0];
      var stats = node.stats;

      expect$3(stats['foo']['a']).to.equal(1, 'we incremented \'a\'');
      expect$3(stats['bar']['e']).to.equal(1, 'we incremented \'e\'');
      expect$3(stats['bar']['d']).to.equal(0, 'we didn\'t increment \'d\'');
      ...
      expect$3(node.stats['foo']).to.equal(undefined, 'we reset');
    });
  });
});
...
var now$2 = Date.now || function() { return new Date().getTime(); };
...
describe('heimdall', function () {
  ...
  it('creates a new session if none is provided', function () {
    ...
    expect$4(h1._events.get(4)[1]).to.equal('a');
    expect$4(h2._events.get(4)[1]).to.equal('b');
  });
  ...
  describe('.start/stop/resume', function () {
    ...
    it('supports basic start/stop', function () {
      expect$4(tree.path).to.eql(['session-root']);

      var tokenA = heimdall.start('node-a');
      expect$4(tree.path).to.eql(['session-root', 'node-a']);

      var tokenB = heimdall.start('node-b');
      expect$4(tree.path).to.eql(['session-root', 'node-a', 'node-b']);
    ...
    it('restores the node at time of resume', function () {
      ...
      expect$4(tree.path).to.eql(['session-root', 'node-a', 'node-c']);
  ...
  describe('monitors', function () {
    ...
    it('hasMonitor returns false if a given namespace is not being used', function () {
      expect$4(heimdall.hasMonitor('some-monitor')).to.eql(false);
    });
    ...
    it('hasMonitor returns true if a given namespace is being used', function () {
      ...
      expect$4(heimdall.hasMonitor('some-monitor')).to.eql(true);
    });
    ...
    it('should allow to aggregate stats', function () {
      var _heimdall$registerMon3 = heimdall.registerMonitor('my-monitor', 'a', 'b');

      var a = _heimdall$registerMon3.a;
      var b = _heimdall$registerMon3.b;
...
describe('HeimdallTree', function () {
  describe('when constructing the tree', function () {
    ...
    it('sets the root node to "page-root"', function () {
      ...
      expect$5(tree.root.name).to.equal('page-root');
    ...
    it('properly adds children', function () {
      ...
      expect$5(tree.root.leaves.length).to.equal(2);
      expect$5(tree.root.nodes.length).to.equal(1);
      expect$5(tree.root.children.length).to.equal(3);

      var child = tree.root.nodes[0];
...
describe('HeimdallNode', function () {
  ...
  describe('visiting', function () {
    ...
    it('.visitPreOrder visits nodes depth first pre-order', function () {
      ...
      expect$7(path).to.eql(['page-root', 'A', 'B', 'C', 'D']);
    });
    ...
    it('.visitPostOrder visits nodes depth first post-order', function () {
      ...
      expect$7(path).to.eql(['C', 'B', 'D', 'A', 'page-root']);
    });

    it('forEachNode visits each child node only', function () {
      var path = [];
      var baseNode = root.nodes[0];
